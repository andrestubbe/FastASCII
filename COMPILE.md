# Building FastASCII from Source
...
- Uses `native\FastASCII.def` for JNI exports
- Outputs to `build\FastASCII.dll`

The Maven `pom.xml` will automatically pick up `build\FastASCII.dll` and bundle it inside the JAR.

## JNI Exports (.def File)

When using JNI, you MUST export your native functions in the `native\FastASCII.def` file:

```def
LIBRARY FastASCII
EXPORTS
    Java_FastASCII_FastASCII_doSomethingNative
```
